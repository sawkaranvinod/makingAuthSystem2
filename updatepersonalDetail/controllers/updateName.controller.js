...
import {grpcUpdateName} from "../grpc/utils/updateName.utils.js";
config();
const accessTokenKey = process.env.ACCESS_TOKEN_KEY || "hellobro";
export async function handleUpdateName(req,reply) {
    try {
      let {email,name,accessToken,deviceFingerPrint} = req.body;  
       const verifyAccessToken = await verifyToken(accessToken,accessTokenKey);
        ...
        let response;
        try {
           response = await grpcUpdateName(email,name); 
        } catch (error) {
           console.log("error is coming from the grpc server of the update name function",error.message);
        ...
        console.log("error in the handler function of the update name controller function",error.message);
        return reply.send({message:"internal server error"}).status(500);
    }
}